...
stepper = startpoint.copy()
dis_dict1 = {}
dis1 = 0
#for dir in ['R98','U47','R26','D63','R33','U87','L62','D20','R33','U53','R51']:
for dir in inputs[0]:
    direct, steps = dir[0], int(dir[1:])
    if direct == 'U':
        for s in range(steps):
            stepper[1] -= 1
            dis1 += 1
            if stepper in coord:
                dis_dict1[(stepper[0], stepper[1])] = dis1
    elif direct == 'R':
        for s in range(steps):
            stepper[0] += 1
            dis1 += 1
...
for dir in inputs[1]:
    ...
    if direct == 'U':
        for s in range(steps):
            ...
            dis2 += 1
            if stepper in coord:
                dis_dict2[(stepper[0], stepper[1])] = dis2
    elif direct == 'R':
        for s in range(steps):
            stepper[0] += 1
            dis2 += 1
...
for key, value in dis_dict1.items():
    distances2.append(value+dis_dict2[key])
